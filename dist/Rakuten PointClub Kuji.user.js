...
(() => {
  ...
  window.addEventListener("load", () => {
    if (location.search) window.open("about:blank", "_self")?.close();
    const target = document.getElementById("tametoku-mode-kuji-contents");
    if (!target) throw new Error("Target not found.");
    new MutationObserver((mutations, observer) => {
      for (const mutation of mutations)
        if (mutation.target instanceof HTMLElement) {
          const es1 = Array.from(
            mutation.target.querySelectorAll(".is-pc > .cta_btn")
          ).filter((e) => e instanceof HTMLElement).filter((e) => visible(e));
          for (const e1 of es1) e1.click();
          const es2 = Array.from(
            mutation.target.querySelectorAll(
              ".is-pc > .js-tametoku-mode-kuji-start-btn"
            )
          ).filter((e) => e instanceof HTMLElement).filter((e) => visible(e));
