...
(() => {
  ...
  function equiv(urlLike1, urlLike2) {
    ...
    if (url1.pathname.replaceAll(/\/\/+/g, "/").replace(/\/$/, "") !== url2.pathname.replaceAll(/\/\/+/g, "/").replace(/\/$/, ""))
      return false;
  ...
  if (window.top === window.self)
    window.addEventListener("load", async () => {
      ...
      function assign(urlLike) {
        ...
        if (hasLocalePrefix(location) && !hasLocalePrefix(url) || !hasLocalePrefix(location) && hasLocalePrefix(url))
          return;
...
${url.href}`,
      ...
      async function getCanonical() {
        ...
        async function normalize() {
          ...
          if (paths.at(-1)?.length === 0 || paths.at(-1)?.match(/index\.\w+/))
            paths.pop();
          const result = await tryFetch(
            `${url.origin}${paths.join("/")}${url.search}${url.hash}`
          );
          return result.exists ? result.url : null;
          async function fetchShortened() {
            const parsed = tldts.parse(location.origin);
            if (parsed.subdomain !== "www") return location.href;
