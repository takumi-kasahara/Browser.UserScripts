  ...
  function extractObjects(data) {
    ...
    function visit(data2) {
      ...
      if (Array.isArray(data2))
        return data2.flatMap((item) => visit(item));
      const children = Object.values(data2).flatMap(
        (value) => visit(value)
      );
  ...
  if (window.top === window.self)
    window.addEventListener("load", () => {
      ...
      new MutationObserver((mutations) => {
        ...
        function handle(target, addedNodes, ...reason) {
          ...
          for (const script of scripts) {
            ...
            for (const data of extractObjects(
              tryParse(script.textContent?.trim() ?? "")
            )) {
              console.log("JSON Linked Data:", data["@type"], data);
              fromJson(data);
            }
      ...
      function fromJson(data) {
        if (Object.hasOwn(data, "headline")) setTitle(String(data["headline"]));
